...
const AddContact = () => {
  ...
  const imagePicker = async e => {
    ...
    try{
        ...
        console.error(error)
        toast('Spmething Went Wrong', {type:'error'})
    }
  };
  ...
  const addContact = async () => {
    ...
    try {
      firebase.database().ref('contacts/' + v4()).set({name, email, phoneNumber,address,picture:downloadUrl,star})

    } catch (error){
      console.log(error)
      toast('0pps..', {type:"error"})
    }
  };
